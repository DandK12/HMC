...
import { exportLeaveRequests } from '../../../utils/exports/leaveRequestExport';
...
export function LeaveRequests() {
  ...
  const handleApprove = async (request: LeaveRequest) => {
    try {
      ...
      setError('Gagal menyetujui permintaan cuti');
      console.error(err);
    }
  };
  ...
  const handleReject = async (request: LeaveRequest) => {
    try {
      ...
      setError('Gagal menolak permintaan cuti');
      console.error(err);
    }
  };

  const handleDelete = async (request: LeaveRequest) => {
    if (!confirm('Apakah kamu yakin ingin menghapus permintaan cuti ini?')) {
      return;
    }
    ...
    try {
      ...
      setError('Gagal menghapus permintaan cuti');
      console.error(err);
    }
  };
  ...
  const handleEditSave = async () => {
    ...
    try {
      ...
      setError('Gagal memperbarui permintaan cuti');
      console.error(err);
    }
  };
  ...
  const handleExportCSV = () => {
    try {
      exportLeaveRequests(requests, employees);
    } catch (err) {
      setError('Gagal mengekspor permintaan cuti');
      console.error(err);
    }
  };
      ...
      <div className="p-6 space-y-6">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-2xl font-bold text-[#105283]">Permintaan Cuti</h2>
          </div>
