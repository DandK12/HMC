...
export function TimeTracking() {
  ...
  const handleImport = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Gagal import waktu duty');
    } finally {
      setImporting(false);
      if (event.target) {
        event.target.value = '';
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Gagal menghapus waktu duty');
    }
  };

  const handleSaveTimeEntry = async (updatedEntry: Partial<WorkingHours>) => {
    if (!selectedEmployee) {
      throw new Error('Tidak ada anggota yang dipilih');
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <ConfirmationDialog
        ...
        title="Hapus Waktu Duty"
        message="Apakah kamu yakin ingin menghapus waktu duty ini?"
        confirmLabel="Hapus"
        cancelLabel="Batal"
      />
    </div>
