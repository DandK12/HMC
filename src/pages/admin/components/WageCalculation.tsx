  ...
  const handleExportExcel = () => {
    ...
    const exportData = wageData.map(data => {
      const baseData = {
        ...
        'Total Hours': data.isFixedSalary ? 'Fixed Salary' : data.monthlyHours.toFixed(1),
    ...
    utils.book_append_sheet(workbook, worksheet, 'Wage Calculation');

    // Generate filename with month and year
    const [year, month] = selectedMonth.split('-');
    const filename = `wage_calculation_${month}_${year}.xlsx`;
